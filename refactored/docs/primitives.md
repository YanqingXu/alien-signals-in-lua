...
- `computedNeedsRefresh` 返回 true：调用 `updateComputed(node, true)`，
  把旧值作为参数传给 getter；若新值变了，对下游 `markDirty`。
- 节点 `isInactive`（首次被读）：调用 `initComputed`，
  不带旧值地执行 getter，并把结果存进 `value`。

之后必然调用 `trackRead(node)`，让 computed 自己成为
...
- 如果当前 active subscriber 存在（嵌套场景），调用
  `graph.connect(effectNode, parent, 0)` 把当前 effect
...
- 用 `callWithSub` 在 `pcall` 里执行 `fn`：返回值若是函数，作为下次
  重跑前/停止时的 cleanup 保存到 `effectNode.cleanup`。
- 失败时调用 `stopScopeNode` 把节点彻底拆除并上抛错误。
- 返回 `bind(stopEffect, effectNode)`：用户调用即停止 effect、运行 cleanup。
...
- 返回的 stop callable 会清空 scope 自己的依赖与所有子订阅者
  （子 effect 在被 `graph.unlink` 摘除后，因 subs 变空触发
  `engine.handleUnwatched` → `stopNode` →
  `stopEffect` 链式停止并运行 cleanup）。
...
unlinkDepsReverse(scopeNode) -- 逆序摘掉上游；子 effect 会先 cleanup
while scopeNode.subs do                   -- 摘掉自己的所有下游
    unlink(scopeNode.subs)
end
```

注意第二个循环依赖 `unlink` 摘掉 `link` 后会让
`scopeNode.subs` 自动指向 `nextSub`，所以无需手动推进。

`stopEffect` 在此基础上多跑一次 cleanup。注意顺序是先停子树，再跑父 effect
自己的 cleanup：
...
`stopNode` 是注入给 engine 的分发入口：effect 走 `stopEffect`，
scope 走 `stopScopeNode`。
...
sequenceDiagram
    ...
    Pr->>Eg: propagate(count.subs) [PUSH]
...
| ② PUSH | `propagate` | 沿 `subs` 打 `Pending`、收集 effect 入队 | 不调用任何 getter |
| ③ 调度 | `flush()`（或 `endBatch`） | 出队、调用 `runQueuedEffect` | — |
| ④ PULL | `checkDeps` | 沿 `deps` 递归确认、必要时重算 computed | 不传播"未变"的标记 |
...
- **`effect` 的初始 `RecursedCheck` 哪里去掉？** 在原语函数体内执行完
  ...
  调用 `graph.unlinkStaleDeps(effectNode)`——这等价于
  `engine.finishTrack`，但因为构造期不走 `engine.beginTrack`，
  所以在这里手动收尾。
- **`trigger` 为什么要先做一个 fake subscriber？** 这样能复用现有的依赖
  收集机制（`trackRead → graph.connect`），
