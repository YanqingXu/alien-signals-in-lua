...
- `callWithSub(sub, fn)`：在 `pcall` 里推/弹 active subscriber，
  同时增减 `runDepth`。返回 `(ok, result)`，错误传给调用方决定如何处理。
- `trackRead(dep)`：当 `activeSubscriber` 存在时，把
  `dep → activeSubscriber` 这条边交给 `graph.connect`，
  并带上当前的 `trackingVersion`。
- `beginTrack(sub, baseFlags, advanceVersion)` /
  `finishTrack(sub)`：effect/computed 重跑的标准包裹流程——
  推进 version、置空 `depsTail`、加 `RecursedCheck`；跑完后摘掉
  `RecursedCheck`，并调用 `graph.unlinkStaleDeps` 清旧依赖。

### 2. 失效传播 — `propagate`（PUSH 阶段）
...
#### 单步决策：`decidePropagation(sub, link, isWriteInsideRun)`
...
2. 该 subscriber **完全干净** (`PROPAGATION_GUARD_FLAGS` 全 0)：加上
   ...
   根据旧 flags 返回 `ScheduleEffect` / `VisitChildren` 动作。
...
5. 该 subscriber 在 `RecursedCheck` 中、还不脏，且本次 link 落在
   `isLinkInCurrentDeps` 范围内：补上 `Recursed | Pending`，
   仅返回 `VisitChildren`，让 Mutable 节点继续向下游传播。
6. 否则不传播。

#### 单步动作：`processInvalidated`

- 若决策结果含 `ScheduleEffect` —— 把 effect 入队 (`scheduler.enqueueEffect`)。
- 若决策结果含 `VisitChildren` —— 返回 `subscriber.subs` 作为"下一层 link"，让
  外层循环继续向下扩散。

#### 外层遍历：手写显式栈代替递归

`propagate` 用 `currentLink` / `nextLink` / `stack` 三个变量
实现深度优先遍历。深入到下游时，把当前层级的 `nextLink` 推入栈，回溯时再
弹回。这避免了真递归带来的 Lua 栈深限制，也方便在错误情况下断点调试。

### 3. 脏值检查 — `checkDeps`（PULL 阶段）
...
flowchart TD
    ...
    Q1 -- 是 --> U1[updateNode dep<br/>提交新值 / 重算 computed]
    U1 --> Q1V{值真的变了?}
    Q1V -- 是 --> M1[向下游<br/>markDirty]
    M1 --> RT2([return true]):::ok
    Q1V -- 否 --> NEXT1[继续下一条 link]

    Q0 -- 否 --> Q2{dep 是<br/>Mutable + Pending?}
    Q2 -- 是 --> REC[递归调用<br/>checkDeps<br/>dep.deps, dep]
    REC --> Q2V{递归返回 true?}
    Q2V -- 是 --> U2[updateNode dep] --> Q2VV{值真的变了?}
    Q2VV -- 是 --> M2[向下游 markDirty] --> RT3([return true]):::ok
...
- **首次发现"真的变了"就立即返回 true**，不需要再看后面的 link——调用方
  （`computedNeedsRefresh` / `shouldRunEffect`）会据此决定重算。
...
- **重算后还要 `markDirty`**：把"可能脏 (Pending)"升级为
...
- `commitSignalValue` —— 把 `pendingValue` 落到 `currentValue`，返回是否
  变化。值相等时不触发下游。
- `updateComputed` —— 如果 computed 拥有子 effect，先按逆序释放旧子树；
  再用 `beginTrack` 包裹 getter 调用，失败时给节点打上 `Dirty`
  以便下次仍尝试重跑，再 `error` 上抛。
- `initComputed` —— 首次激活 lazy computed；和 update 的差别在
  不推进 `trackingVersion`（首次还没有"上一轮"可比较）。
- `runEffectBody` / `runCleanup` / `runQueuedEffect` —— effect 的完整
  重跑流程；`runQueuedEffect` 是注入给 `scheduler` 的入口函数。

### 5. 资源回收 — `handleUnwatched`

由 `graph` 在依赖源 `subs` 变为空时回调：

- 非 Mutable 节点（effect / scope）：调用 `stopNode(node)` 真正停掉。
...
`primitives` 会给父节点加上 `HAS_CHILD_EFFECT`；`engine.runQueuedEffect`
...
再把 `stopNode` 注入到 `engine.setStopHandler`，
...
- **反向注入**：`engine.runQueuedEffect → scheduler`、
  ...
  `primitives.stopNode → engine`。

## 关键细节回顾

- **`runDepth` 的用途**：`primitives.signal` 写入时调用
  `engine.isInsideReactiveRun()` 来判定 "本次写入是否发生在 effect 重跑过程
  中"，并把结果传给 `propagate`，决定是否在传播路径上
  补 `Recursed` 标志。
- **`trackingVersion` 为什么是全局递增？** 它必须跨节点共享，才能让
  `graph.connect` 在"同一次重跑内同一依赖被读两次"
  时去重。
- **错误恢复**：每个外部调用 (`updateComputed` / `runEffectBody` /
  `runCleanup`) 都把 `activeSubscriber` 与 `runDepth` 在 `callWithSub`
